...
Jusqu'ici, nous avons exploré différentes attaques informatiques ainsi que les mécanismes de sécurité permettant de s'en prémunir. Ces mécanismes visent tous un même objectif fondamental : établir de la confiance dans les systèmes numériques.

Mais la confiance ne se résume pas seulement à protéger des données contre des accès non autorisés. Elle soulève une question plus profonde : comment s'assurer que le comportement d'une application s'exécute exactement comme prévu, sans qu'aucun acteur malveillant ni même l'opérateur du service ne puisse en détourner le fonctionnement ?

Prenons un exemple concret : imaginons un service de cagnotte commune entre amis. Ce service tourne sur un serveur centralisé. Si un attaquant venait à prendre le contrôle de ce serveur, ou si l'opérateur lui-même agissait de manière malhonnête, rien n'empêcherait techniquement de modifier les règles du jeu : falsifier les soldes, détourner les fonds, ou altérer la logique de distribution. La confiance repose alors entièrement sur une tierce partie et c'est précisément là que réside la faiblesse.

C'est pour répondre à ce problème que la blockchain a été conçue. Dans sa version initiale (Bitcoin, 2008), elle permet de maintenir un registre distribué et infalsifiable de transactions, sans avoir besoin de faire confiance à un serveur central. Mais c'est avec l'arrivée de la blockchain 2.0, incarnée par Ethereum, qu'émerge le concept de smart contract (contrat intelligent) avec des programmes autonomes déployés sur la chaîne de blocs, dont le code et l'exécution sont publics, transparents et immuables. Une fois déployé, un smart contract s'exécute exactement comme son code le stipule, personne pas même son créateur, ne peut en modifier le comportement.

Pour bien appréhender les concepts abordés dans ce TP, vous pouvez consulter le document blockchain.pdf fourni avec ce sujet, ou visionner cette excellente vidéo d'introduction : https://www.youtube.com/watch?v=bBC-nXj3Ng4

**1) Expliquez pourquoi la blockchain est considérée comme infalsifiable. Quelles sont les méthodes cryptographiques qui garantissent cette propriété ?**

**2) Qu'est-ce que la preuve de travail (Proof of Work) et la preuve d'enjeu (Proof of Stake) ? Laquelle des deux est la moins énergivore, et pour quelle raison ?**

**3) Quel est l'intérêt des smart contracts d'un point de vue sécurité ?**

**4) La blockchain est-elle pour autant invulnérable ? Si non, quelles types d'attaques peut-elle subir ?**
...
**QA.8 Quel est le rôle du genesis block dans une blockchain ?**
